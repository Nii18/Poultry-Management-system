                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        @foreach($sales as $sale)
                                            <tr>
                                                <td>{{ $sale['name'] }}</td>
                                                <td>{{ $sale['subtitle'] ?? 'N/A' }}</td>
                                                <td>₵{{ number_format($sale['amount'] ?? 0, 2) }}</td>
                                                <td>
                                                    <button type="button"
                                                            class="btn btn-sm btn-success"
                                                            data-id="{{ $sale['id'] }}"
                                                            onclick="showSaleDetails(this)">
                                                        <i class="fas fa-eye"></i> View
                                                    </button>
                                                </td>
                                            </tr>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <h4 class="header-title mb-3">
                                ...
                                Breeding Records ({{ $breeding_records->count() }})
                            </h4>
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    <thead>
                                    ...
                                    <tbody>
                                        @foreach($breeding_records as $record)
                                            <tr>
                                                <td>{{ $record['name'] }}</td>
                                                <td>{{ $record['subtitle'] ?? 'N/A' }}</td>
                                                <td>
                                                    <a href="{{ $record['url'] ?? route('breeding-records.index') }}"
                                                       class="btn btn-sm" style="background:#ec4899;color:white;">
                                                        <i class="fas fa-eye"></i> View
                                                    </a>
                                                </td>
                                            </tr>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title"><i class="fas fa-eye me-2"></i>Flock Details</h5>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-success text-white">
                <h5 class="modal-title"><i class="fas fa-eye me-2"></i>House Details</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-danger text-white">
                <h5 class="modal-title"><i class="fas fa-eye me-2"></i>Treatment Details</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-info text-white">
                <h5 class="modal-title"><i class="fas fa-eye me-2"></i>Daily Log Details</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-warning text-dark">
                <h5 class="modal-title"><i class="fas fa-eye me-2"></i>Expense Details</h5>
...
<div class="modal fade" id="viewSaleModal" tabindex="-1" data-bs-backdrop="static">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-success text-white">
                <h5 class="modal-title"><i class="fas fa-chart-line me-2"></i>Sale Details</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
...
function spinnerHtml(colorClass) {
...
window.showFlockDetails = function(btn) {
    ...
    modalBody.innerHTML = spinnerHtml('text-primary');
    modal.show();
    ...
    .then(data => {
        if (data.success) {
            ...
            modalBody.innerHTML = `<div class="alert alert-danger">Failed to load flock details: ${escapeHtml(data.message)}</div>`;
        }
    })
    .catch(() => {
...
window.showHouseDetails = function(btn) {
    ...
    modalBody.innerHTML = spinnerHtml('text-success');
    modal.show();
    ...
    .then(data => {
        if (data.success) {
            ...
            modalBody.innerHTML = `<div class="alert alert-danger">Failed to load house details: ${escapeHtml(data.message)}</div>`;
        }
    })
    .catch(() => {
        modalBody.innerHTML = `<div class="alert alert-danger">Error loading data. Please try again.</div>`;
...
window.showTreatmentDetails = function(btn) {
    ...
    modalBody.innerHTML = spinnerHtml('text-danger');
    modal.show();
    ...
    .then(data => {
        if (data.success) {
            ...
            modalBody.innerHTML = `<div class="alert alert-danger">Failed to load treatment details: ${escapeHtml(data.message)}</div>`;
        }
    })
    .catch(() => {
        modalBody.innerHTML = `<div class="alert alert-danger">Error loading data. Please try again.</div>`;
...
window.showDailyLogDetails = function(btn) {
    ...
    modalBody.innerHTML = spinnerHtml('text-info');
    modal.show();
    ...
    .then(data => {
        if (data.success) {
            ...
            modalBody.innerHTML = `<div class="alert alert-danger">Failed to load log details: ${escapeHtml(data.message)}</div>`;
        }
    })
    .catch(() => {
        modalBody.innerHTML = `<div class="alert alert-danger">Error loading data. Please try again.</div>`;
...
window.showExpenseDetails = function(btn) {
    ...
    modalBody.innerHTML = spinnerHtml('text-warning');
    modal.show();
    ...
    .then(data => {
        if (data.success) {
            ...
            modalBody.innerHTML = `<div class="alert alert-danger">Failed to load expense details: ${escapeHtml(data.message)}</div>`;
        }
    })
    .catch(() => {
        modalBody.innerHTML = `<div class="alert alert-danger">Error loading data. Please try again.</div>`;
...
// ──────────────────────────────────────────────────────────────
// SALE MODAL
// Hits /sales/{id}/details-json  →  SaleController@getDetailsJson
//
// JSON shape (from controller):
//   sale.id, sale.sale_date, sale.product_type (label),
//   sale.product_type_key, sale.quantity, sale.unit_price,
//   sale.total_amount, sale.customer_name, sale.payment_method,
//   sale.receipt_number, sale.description, sale.flock_number,
//   sale.flock_breed, sale.notes, sale.recorded_by, sale.created_at
// ──────────────────────────────────────────────────────────────
window.showSaleDetails = function(btn) {
    ...
    fetch(`/sales/${saleId}/details-json`, {
        headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' }
    })
    .then(r => r.json())
    .then(data => {
        if (data.success) {
            ...
            modalBody.innerHTML = `
                <div class="detail-section">
                    <div class="d-flex justify-content-between align-items-start mb-3">
                        <div>
                            <h5 class="mb-1">${escapeHtml(s.product_type)}</h5>
                            <p class="text-muted mb-0">
                                ${s.customer_name && s.customer_name !== 'N/A'
                                    ? escapeHtml(s.customer_name)
                                    : 'No customer name recorded'}
                            </p>
                        </div>
                        <span class="badge badge-amount px-3 py-2">
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-4">
                        <div class="text-center p-3 bg-light rounded">
                            <div class="fs-5 fw-bold text-success">₵${escapeHtml(s.total_amount)}</div>
                            <small class="text-muted">Total Amount</small>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="text-center p-3 bg-light rounded">
                            <div class="fs-5 fw-bold text-primary">${escapeHtml(s.quantity)}</div>
                            <small class="text-muted">Quantity</small>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="text-center p-3 bg-light rounded">
                            <div class="fs-5 fw-bold text-info">₵${escapeHtml(s.unit_price)}</div>
                ...
                <div class="detail-section">
                    ...
                    <div class="detail-grid">
                        <div class="detail-item">
                            ...
                            <span class="detail-value">${escapeHtml(s.sale_date)}</span>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">Product Type</span>
                            <span class="detail-value">${escapeHtml(s.product_type)}</span>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">Customer</span>
                            <span class="detail-value">${escapeHtml(s.customer_name)}</span>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">Payment Method</span>
                            <span class="detail-value">${escapeHtml(s.payment_method)}</span>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">Receipt Number</span>
                            <span class="detail-value">${escapeHtml(s.receipt_number)}</span>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">Flock</span>
                            <span class="detail-value">
                                ${s.flock_number
                                    ? 'Flock #' + escapeHtml(s.flock_number) + (s.flock_breed ? ' · ' + escapeHtml(s.flock_breed) : '')
                                    : 'N/A'}
                            </span>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">Recorded By</span>
                            <span class="detail-value">${escapeHtml(s.recorded_by)}</span>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">Recorded At</span>
                            <span class="detail-value">${escapeHtml(s.created_at)}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="detail-section">
                    <h6>Description</h6>
                    <p class="mb-0 p-3 bg-success rounded">${escapeHtml(s.description)}</p>
                </div>` : ''}

                ${s.notes ? `
                <div class="detail-section">
                    <h6>Notes</h6>
                    <p class="mb-0 p-3 bg-success rounded">${escapeHtml(s.notes)}</p>
                </div>` : ''}
            `;
        } else {
            modalBody.innerHTML = `<div class="alert alert-danger">Failed to load sale details: ${escapeHtml(data.message)}</div>`;
        }
    })
    .catch(() => {
        modalBody.innerHTML = `<div class="alert alert-danger">Error loading data. Please try again.</div>`;
